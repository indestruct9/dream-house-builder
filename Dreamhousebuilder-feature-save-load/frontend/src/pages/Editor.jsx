...
/**
 * Day15 Editor — versions + collab + presence + op pending indicators
 *
...
export default function Editor() {
  ...
  const [collabStatus, setCollabStatus] = useState("idle"); // idle | connecting | syncing | connected
  const [participants, setParticipants] = useState([]); // array of {userId, displayName, cursor, joinedAt}
  const [pendingOps, setPendingOps] = useState({}); // opId -> {op, opId, roomName, ts}
  ...
  // ---------- Collab lifecycle ----------
  useEffect(() => {
    // initialize collab when projectId exists
    if (!projectId) {
    ...
    const client = new CollabClient({
      ...
      onSnapshot: (remoteLayout, clients) => {
        // remote snapshot should replace local canvas (no history)
      ...
      onOp: (msg) => {
        // received op from server
        const op = msg.op;
        const opId = msg.opId;
        // if this op matches a pending local op, clear it
        if (opId && pendingOps[opId]) {
          setPendingOps((prev) => {
            const cp = { ...prev };
            delete cp[opId];
            return cp;
        ...
        // apply op without pushing to history (remote)
    ...
    // optional: send presence periodically (e.g., cursor or camera pos from viewer)
    const presenceTimer = setInterval(() => {
      try {
        // example simple presence: send no cursor (placeholder)
    ...
    return () => {
      ...
      try { client.requestSave(); } catch (e) {}
  ...
  const applyOpToLayout = (op, { pushHistory = false } = {}) => {
    ...
    setLayout((prev) => {
      ...
      if (op.kind === "room:add") {
        ...
        const name = op.name;
        next.rooms = next.rooms.filter((r) => r.name !== name);
  ...
  const _sendOpWithPending = (op, roomNameForUI) => {
    ...
    if (collabRef.current) {
      collabRef.current.sendOp({ ...op, opId }).then(() => {
        // server echoed, handled in onOp callback (we still clear here if necessary)
        setPendingOps((prev) => {
          const cp = { ...prev };
          delete cp[opId];
          return cp;
        ...
        // on error, clear pending after timeout
        setTimeout(() => {
          setPendingOps((prev) => {
            const cp = { ...prev };
            delete cp[opId];
      ...
      // no collab, resolve immediately (local only)
      setTimeout(() => {
        setPendingOps((prev) => {
          const cp = { ...prev };
          delete cp[opId];
  ...
  const handleTransformEnd = (roomName, { x, y, rotationY, scale }) => {
    ...
    const op = { kind: "room:update", room: { name: roomName, x: Number(x), y: Number(y), rotationY: rotationY ?? 0, scale: scale ?? 1 } };
  ...
  return (
    <div className="p-6 grid grid-cols-3 gap-6">
      <div className="col-span-1 space-y-4">
        ...
        <div className="mt-3">
          <div className="text-sm">Collab status: <strong>{collabStatus}</strong></div>
        ...
        <div className="mt-4">
          ...
          <div style={{ display: "flex", gap: 8, flexDirection: "column" }}>
            {participants.length === 0 ? <div style={{ color: "#666" }}>No one else here</div> : participants.map((p) => (
              <div key={p.userId} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <div style={{ width: 32, height: 32, borderRadius: 16, background: "#ddd", display: "flex", alignItems: "center", justifyContent: "center" }}>{(p.displayName || "U")[0]}</div>
                <div style={{ fontSize: 13 }}>
                  <div style={{ fontWeight: 600 }}>{p.displayName}</div>
                  <div style={{ fontSize: 11, color: "#666" }}>{p.cursor ? `cursor: ${JSON.stringify(p.cursor)}` : "idle"}</div>
                </div>
              </div>
            ))}
          </div>
